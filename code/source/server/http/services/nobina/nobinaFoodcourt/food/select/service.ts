...
async function service(req, res, next, options: options) {
    ...
    const restaurant = Tools.isObjectid(req.body.restaurant);
    const category = Tools.isObjectid(req.body.category);
    ...
    try {
        if (true) {
            ...
            req.body.restaurant ? $match.filter['restaurant'] = restaurant : null;
            req.body.category ? $match.filter['category'] = category : null;
            const $project = {
                __v: false
            };
