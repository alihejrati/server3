                    ...
                    .then(async function (Res) {
                        if (!req['_'].user.login) {
                            ...
                            if (user) {
                                ...
                                const foodcourts = await mongodb.aggregate('foodcourt', [
                                    { $match: { owner: ObjectId(req['_'].user.who._id) } },
                                    // { $project: { _id: true } }
                                ], { database: 'nobinaFoodcourt', errorHandler: error => options['service'].error = error });
                                
                                const restaurants = await mongodb.aggregate('restaurant', [
                                    { $match: { owner: ObjectId(req['_'].user.who._id) } },
                                    // { $project: { _id: true, foodcourt: true } }
                                ], { database: 'nobinaFoodcourt', errorHandler: error => options['service'].error = error });
